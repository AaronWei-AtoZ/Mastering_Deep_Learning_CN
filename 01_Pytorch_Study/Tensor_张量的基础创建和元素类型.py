...
线性和随机张量的创建：
    ...
    torch.randn(x, y)      # 正态分布的值在0～1间的x行y列二维张量
...
总结：
    ...
    6.元素类型转变 建议使用：.type() / dtype=tensor.元素类型的方式
...
def linear_tensor():            # 创建线性张量
    t1 = torch.arange(1, 5)     # 1～5的一维张量
    t2 = torch.linspace(1, 20, 3)   # 以1为起点，20为重点，等差值为3的等差数列一维张量
    print(f'张量t1的元素值是:{t1}, t1的类型是:{type(t1)}, 张量t1中的元素值类型是:{t1.dtype}')
    print(f'张量t2的元素值是:{t2}, t2的类型是:{type(t2)}, 张量t2中的元素值类型是:{t2.dtype}')
...
def random_tensor():
    ...
    t2 = torch.randn(3, 4)      # 正态分布的值在0～1间的三行四列二维张量
